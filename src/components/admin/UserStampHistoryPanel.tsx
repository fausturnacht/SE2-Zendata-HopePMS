...
export const UserStampHistoryPanel: React.FC<UserStampHistoryPanelProps> = ({
  userId,
  isOpen,
  colSpan,
}) => {
  const [history, setHistory] = useState<UserStampEntry[] | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        setIsLoading(true);
        setFetchError(null);
        const data = await getUserStampHistory(userId);
        setHistory(data);
      } catch (error) {
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-slate-700">
                Audit trail of all recorded stamp events for this user.
              </p>
            </div>
          </div>
                ...
                <tbody className="divide-y divide-slate-200">
                  {pageItems.length > 0 ? (
                    ...
                    <tr>
                      ...
                      >
                        No stamp history found for this user.
                      </td>
                    </tr>
                  )}
                </tbody>
